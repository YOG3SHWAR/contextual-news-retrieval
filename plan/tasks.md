...
- [ ] **0.1 Maven project + `pom.xml`** — Spring Boot 3.5.x parent, Java 21. Deps:
...
- [ ] **1.1 Schema init** — `articles` + `user_events` tables, PostGIS extension,
      `search_tsv` as a **generated column**, all indexes (GIST geog, GIN tsv,
      GIN `categories_norm`, source, composite score/date `(rank_key, id)`). Via
      Flyway migration or `schema.sql`.
- [ ] **1.2 Entities** — `Article` (geography column via hibernate-spatial,
      `categories` + lowercased `categories_norm` as `text[]`; `search_tsv` is
      DB-generated — map read-only or omit), `UserEvent`.
...
- [ ] **1.4 `DataLoader`** (`ApplicationRunner`) — **stream-parse** the JSON
      ...
      idempotent), populate `categories_norm` (lowercased), set `geog` only when
      lat/lon valid and not `(0,0)` (defensive — the seed file has no such rows),
      skip+log malformed rows, skip load if already populated
...
test-fixture rows with null/`(0,0)` geo get null `geog` (the seed file has none);
ingestion memory is bounded (streaming).
...
- [ ] **2.3 Validation** — `limit` clamp `[1,50]` default 5; `threshold` `[0,1]`
      default 0.7, out-of-range → 400; `lat`/`lon`/`radius` bounds; non-blank
      `category`/`source`/`query`.
- [ ] **2.4 `/category`** (R1) — `categories_norm @> ARRAY[lower(:cat)]` (must use
      the normalized array — `@>` is exact-match, so filtering the original
      mixed-case labels would miss `DEFENCE`/`General`/`IPL_2025`); order pubdate desc.
...
- [ ] **2.7 `SearchProvider` seam + `/search`** (R3, §1.1) — define
      ...
      `0.6*ts_rank(search_tsv, plainto_tsquery(:q), 32) + 0.4*relevance_score`
      (flag 32 normalizes `ts_rank` into `[0,1)` to match the score scale); `ILIKE`
...
- [ ] **3.4 `CacheService`** — layered **local (Caffeine) → Redis** (§10.3), null-safe
      ...
      (`hash(normalizedQuery)` — extraction is location-independent) and summaries
      (`sha256(title+description)`, no TTL).
...
- [ ] **3.6 `QueryService` + `/query`** (R7) — call extractor, resolve location
      entities via a **bundled Indian-city gazetteer** (request lat/lon takes
      precedence; unresolvable → fall back to `search`), **AND-combine**
...
- [ ] **4.2 `EventSimulator`** (`ApplicationRunner`) — synthetic events via
      `EventStream`: hot-article bias, **Indian-city clusters inside the corpus
      bounding box** (lat 15.6–22.5, lon 72.6–80.9 — e.g. Mumbai, Pune, Hyderabad,
      Nagpur), recent-skewed timestamps; dedupe by `event_id`; geohash-7.
- [ ] **4.3 `TrendingService`** — score `Σ typeWeight·e^(-λ·Δt)·geoFactor`
      (view=1/dwell=2/click=3/share=5, 6h half-life); top-N; enrich.
